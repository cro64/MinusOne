...
final class TimelineToolbarView: NSView {
    ...
    override init(frame frameRect: NSRect) {
        ...
        // Tighter than `WindowUI.Metrics.rowSpacing` (8pt) and strictly narrower than `controlBar`'s
        // own between-cluster spacing (`PracticeDeckViewController.buildContent`'s
        // `controlBarSpacing`, 8pt) — this view sits inside that unified control bar, and its own
        // fields must read as more tightly grouped than the gap separating it from its neighbouring
        // clusters, or the grouping the redesign is for falls apart. A local override rather than
        // lowering `rowSpacing` itself, since that constant is shared elsewhere.
        let row = Layout.horizontalStack([label, bpmField, unit, tapButton], spacing: 5)
